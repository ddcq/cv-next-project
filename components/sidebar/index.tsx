...
const Sidebar: FunctionComponent<SidebarProps> = ({ children, id }) => {
	const sidebarVisible = useSelector(selectSidebarVisible);
	return sidebarVisible === id ? <ContentSidebar>{children}</ContentSidebar> : <div />;
};

const ContentSidebar: FunctionComponent = ({ children }) => {
	const [visible, setVisible] = useState(false);
	const dispatch = useDispatch();
	const sidebarContainer = document.getElementById('sidebar-container') || new Element();
	useEffect(() => {
		setVisible(true);
		$('.s_overlay').fadeIn();
	}, []);
